...
void compute_sobel_static()
{
	...
	#pragma omp parallel
	{
		#pragma omp for schedule(static,chunkSize) private(sum,sumx,sumy,thread_id)
		...
		for( int x = 0; x < image_height; ++x ){
			...
			#pragma omp critical
			{
				for(int t=0;t<max;++t){
					if(x==t*chunkSize){
						result_id[count]=thread_id;
						result_start[count]=x;
						count++;
					}
...
int main(int argc, char* argv[])
{
    ...
    max = ceil((float)image_height/chunkSize);
    result_id= new int[max];
    result_start = new int[max];
	...
	std::cout<<"Static time: "<<dtime_static<<std::endl;
    } else {
        double dtime_dyn = omp_get_wtime();
        compute_sobel_dynamic();
        dtime_dyn = omp_get_wtime() - dtime_dyn;
	std::cout<<"Dynamic time: "<<dtime_dyn<<std::endl;
    }

    for(int i=0;i<count;++i){
	std::cout<<"Thread "<<result_id[i]<<" -> Processing Chunk starting at row "<<result_start[i]<<std::endl;
    }
